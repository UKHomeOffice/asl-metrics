        ...
        const queries = ['projects', 'pils', 'training_pils', 'establishments']
          .map(type => {
            return req.db.asl(type)
              .count()
              .where({ status: 'active' })
              .andWhere('issue_date', '<=', req.query.end + ' 23:59:59.999+00') // ignore issued after report period
              .orWhere({ status: 'revoked' })
              .andWhere('issue_date', '<=', req.query.end + ' 23:59:59.999+00')
              .andWhere('revocation_date', '>', req.query.end + ' 00:00:00+00')
              .orWhere(builder => {
                if (type === 'projects' || type === 'training_pils') {
                  return builder.orWhere({ status: 'expired' })
                    .andWhere('issue_date', '<=', req.query.end + ' 23:59:59.999+00')
                    .andWhere('expiry_date', '>', req.query.end + ' 00:00:00+00');
                }
              })
              .where(builder => {
