...
module.exports = ({ db, query: params, flow }) => {
  ...
  const parse = async record => {
    ...
    let project;
    if (model === 'rop') {
      const projectId = get(record, 'data.data.projectId');
      project = await db.asl('projects').where({ id: projectId }).first()
    }
    const schemaVersion = model === 'rop'
      ? project.schemaVersion
      : get(record, 'data.modelData.schemaVersion', 0);
